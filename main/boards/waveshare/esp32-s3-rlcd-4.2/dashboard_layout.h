...
struct DashboardState {
    ...
    lv_obj_t* weather_alert_label = nullptr;
    lv_obj_t* weather_precipitation_label = nullptr;
    ...
    // 当前日期的节日/调休名称。年度名称数据只需要保存当天一项，
    // 这样不会在仪表盘状态中堆积整年的动态字符串。
    int holiday_note_year = 0;
    int holiday_note_day_of_year = 0;
    bool holiday_note_ready = false;
    char holiday_note[64] = {};
};
...
// 刷新日历日期（时间本身由共享 top/status bar 显示）。
...
// 更新当天的节日或调休名称；note 为空时表示当天没有名称化的节日数据。
